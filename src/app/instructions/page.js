    ...
    return (
        <div className="font-sans grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20">
            ...
            <main className="flex flex-col gap-[32px] row-start-2 items-center sm:items-start w-[50vw]">
                ...
                <p>{t("instructions.parameterSelectorInfo")}</p>
                <div className="w-full h-[20vw] relative">
                    <Image
                        fill
                        src={t("instructions.parameterSelectorPicture")}
                        alt="Parameter selector"
                    />
                </div>
                <p className="w-full">{t("instructions.smallMapInfo")}</p>
                <div className="w-full h-[30vw] relative">
                    <Image fill src={t("instructions.smallMapPicture")} alt="Preview map" />
                </div>
                <p className="w-full">{t("instructions.previewTableInfo")}</p>
                <div className="w-full h-[50vw] relative">
                    <Image
                        fill
                        src={t("instructions.previewTablePicture")}
                        alt="Preview map table"
                    />
                </div>
                <p className="w-full">{t("instructions.twoMapsInfo")}</p>
                <div className="w-full h-[15vw] relative">
                    <Image fill src={t("instructions.twoMapsPicture")} alt="Two smaller maps" />
                </div>
            </main>
        </div>
    );
